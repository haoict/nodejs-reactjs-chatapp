...
export default class Connection {
  ...
  sendToMembers(userId, obj) {
    ...
    const users = [];
    this.app.db.collection('channels')
      .aggregate(query, (err, results) => {
        ...
        if (err === null && results) {
          ...
          const memberConnections = this.connections.filter((con) => _.includes(users, _.toString(_.get(con,
            'userId'))));
  ...
  doTheJob(socketId, msg) {
    ...
    switch (action) {
              ...
              this.app.models.channel.load(channelId)
                .then((channel) => {
                  ...
                  _.each(memberIds, (memberId) => {
                    ...
                    const memberConnections = this.connections.filter((c) => _.toString(c.userId) ===
                      memberId);
      ...
      case 'create_channel':
        ...
        this.app.models.channel.create(channel)
          .then((chanelObject) => {
            ...
            this.app.models.user.find(query, queryOptions)
              .then((users) => {
      ...
      case 'auth':
        ...
        if (connection) {
          ...
          this.app.models.token.loadTokenAndUser(userTokenId)
            .then((token) => {
    ...
    this.app.wss.on('connection', (ws) => {
      ...
      ws.on('close', () => {
        ...
        if (userId) {
          ...
          const userConnections = this.connections.filter((con) => _.toString(_.get(con, 'userId')) ===
            userId);
