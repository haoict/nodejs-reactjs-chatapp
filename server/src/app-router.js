...
export default class AppRouter {
  ...
  setupRouter() {
    ...
    app.get('/api/users/me', (req, res, next) => {
      ...
      app.models.token.loadTokenAndUser(tokenId)
        .then((token) => {
    ...
    app.post('/api/users/search', (req, res, next) => {
      ...
      app.models.user.search(keyword)
        .then((results) => {
          return res.status(200)
            .json(results);
        })
    ...
    app.get('/api/channels/:id', (req, res, next) => {
      ...
      app.models.channel.load(channelId)
        .then((channel) => {
          ...
          app.models.user.find(query, options)
            .then((users) => {
              channel.users = users;
              return res.status(200)
                .json(channel);
        ...
        .then((token) => {
          ...
          this.app.models.channel.load(channelId)
            .then((c) => {
              ...
              this.app.models.message.getChannelMessages(channelId, limit, offset)
                .then((messages) => {
                  return res.status(200)
                    .json(messages);
                })
    ...
    app.get('/api/me/channels', (req, res, next) => {
      ...
      app.models.token.loadTokenAndUser(tokenId)
        .then((token) => {
          ...
          app.models.channel.aggregate(query)
            .then((channels) => {
              return res.status(200)
                .json(channels);
            })
